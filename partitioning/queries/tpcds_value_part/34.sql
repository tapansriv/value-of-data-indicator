...
FROM (
    SELECT
      ...
      ss_customer_sk,
      COUNT(*) AS cnt
    FROM READ_PARQUET('store_sales.parquet') AS store_sales, READ_PARQUET('date_dim.parquet') AS date_dim, READ_PARQUET('store.parquet') AS store, READ_PARQUET('household_demographics.parquet') AS household_demographics
    WHERE
    ...
    GROUP BY
      ss_ticket_number,
      ss_customer_sk
) AS dn, READ_PARQUET('customer.parquet') AS customer
WHERE
