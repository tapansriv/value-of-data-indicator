WITH all_sales AS (
    ...
    FROM (
        ...
        JOIN READ_PARQUET('item.parquet') AS item
          ON i_item_sk = cs_item_sk
        JOIN READ_PARQUET('/home/cc/tpcds_partitioned_freq/date_dim/**/*.parquet', hive_partitioning = 1) AS date_dim
          ON d_date_sk = cs_sold_date_sk
        LEFT JOIN READ_PARQUET('catalog_returns.parquet') AS catalog_returns
          ON (
        ...
        LEFT JOIN READ_PARQUET('store_returns.parquet') AS store_returns
          ON (
        ...
        LEFT JOIN READ_PARQUET('web_returns.parquet') AS web_returns
          ON (
