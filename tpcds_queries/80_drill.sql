CREATE TABLE ssr AS
  (SELECT s_store_id AS store_id,
   ...
   LEFT OUTER JOIN dfs.`tmp/store_returns.parquet` AS store_returns ON (ss_item_sk = sr_item_sk
                                     AND ss_ticket_number = sr_ticket_number), dfs.`tmp/date_dim.parquet` AS date_dim,
...
CREATE TABLE csr AS
  (SELECT cp_catalog_page_id AS catalog_page_id,
   ...
   LEFT OUTER JOIN dfs.`tmp/catalog_returns.parquet` AS catalog_returns ON (cs_item_sk = cr_item_sk
                                       AND cs_order_number = cr_order_number), dfs.`tmp/date_dim.parquet` AS date_dim,
...
CREATE TABLE wsr AS
  (SELECT web_site_id,
   ...
   LEFT OUTER JOIN dfs.`tmp/web_returns.parquet` AS web_returns ON (ws_item_sk = wr_item_sk
                                   AND ws_order_number = wr_order_number), dfs.`tmp/date_dim.parquet` AS date_dim,
