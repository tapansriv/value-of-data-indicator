...
FROM
  (SELECT ss_item_sk,
   ...
   LEFT OUTER JOIN dfs.`tmp/store_returns.parquet` AS store_returns ON (sr_item_sk = ss_item_sk
                                     AND sr_ticket_number = ss_ticket_number) ,dfs.`tmp/reason.parquet` AS reason
