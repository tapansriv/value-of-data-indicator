...
TPCDS_DIR = os.path.join(HOME, "tpcds_cluster_base")

VALUE_DIR = Path("../queries/tpcds_value_part")
FREQ_DIR = Path("../queries/tpcds_freq_part")
RAND_DIR = Path("../queries/tpcds_rand_part")
...
def validate_directory(con, directory, label):
    ...
    for sql_file in sorted(directory.glob("*.sql")):
        ...
        try:
            ret = con.execute(f"EXPLAIN {sql}").fetchall()
...
def main():
    ...
    total_failed += validate_directory(con, RAND_DIR, "RAND")
