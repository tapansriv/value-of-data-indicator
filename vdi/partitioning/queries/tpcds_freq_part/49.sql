...
FROM (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM READ_PARQUET('web_sales.parquet') AS ws
            LEFT OUTER JOIN READ_PARQUET('web_returns.parquet') AS wr
              ON (
                ws.ws_order_number = wr.wr_order_number AND ws.ws_item_sk = wr.wr_item_sk
              ), READ_PARQUET('/home/cc/tpcds_partitioned_freq/date_dim/**/*.parquet', hive_partitioning = 1) AS date_dim
            WHERE
    ...
    FROM (
        ...
        FROM (
            ...
            FROM READ_PARQUET('catalog_sales.parquet') AS cs
            LEFT OUTER JOIN READ_PARQUET('catalog_returns.parquet') AS cr
              ON (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM READ_PARQUET('/home/cc/tpcds_partitioned_freq/store_sales/**/*.parquet', hive_partitioning = 1) AS sts
            LEFT OUTER JOIN READ_PARQUET('store_returns.parquet') AS sr
              ON (
