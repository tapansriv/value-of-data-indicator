...
def parse_query_logs():
    ...
    while curr_line < num_lines: 
        ...
        query = None
        num_skiped = 0
        if not lines[curr_line+1].startswith("2025-"):
            # query is multiline
            query_parts = [query_start]
            i = 1
            while not lines[curr_line+i].startswith("2025-"):
...
for i in range(1, 132):
    ...
    if not os.path.isdir(dir_):
        print(f"{i}.sql failed")
        continue
    os.chdir(dir_)
    with open(f"sqlline_queries.json") as fp:
        data = json.load(fp)
        if data["outcome"] == "FAILED":
            print(f"{i}.sql failed")
        elif data["outcome"] == "COMPLETED":
            print(f"{i}.sql completed")
            completed.append(i)
            parse_query_logs()
        else:
...
with open("tpcds_completed.json", 'w') as fp:
    json.dump(completed, fp, indent=4)
