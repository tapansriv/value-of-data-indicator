WITH ssr AS (
    ...
    ) AS salesreturns, READ_PARQUET('/home/cc/tpcds_cluster_base/date_dim/*.parquet') AS date_dim, READ_PARQUET('/home/cc/tpcds_cluster_base/store/*.parquet') AS store
    WHERE
      date_sk = d_date_sk
    ...
    ) AS salesreturns, READ_PARQUET('/home/cc/tpcds_cluster_base/date_dim/*.parquet') AS date_dim, READ_PARQUET('/home/cc/tpcds_cluster_base/catalog_page/*.parquet') AS catalog_page
    WHERE
      date_sk = d_date_sk
      AND d_date BETWEEN CAST('2000-08-23' AS DATE) AND CAST('2000-09-06' AS DATE)
    ...
    FROM (
        ...
        LEFT OUTER JOIN READ_PARQUET('/home/cc/tpcds_cluster_value/web_sales/*.parquet') AS web_sales
          ON (
            wr_item_sk = ws_item_sk AND wr_order_number = ws_order_number
          )
    ) AS salesreturns, READ_PARQUET('/home/cc/tpcds_cluster_base/date_dim/*.parquet') AS date_dim, READ_PARQUET('/home/cc/tpcds_cluster_base/web_site/*.parquet') AS web_site
    WHERE
      date_sk = d_date_sk
      AND d_date BETWEEN CAST('2000-08-23' AS DATE) AND CAST('2000-09-06' AS DATE)
